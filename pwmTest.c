...
void tempControl(){
	int direction = ReadSwitch();
	int data = jp1Controller->data;
	if(direction){
		GPIO_Set(PHASE_PIN, 1);
		GPIO_Set(ENABLE_PIN, 1);
		printf("on: %d\n", direction);
	}else{
		GPIO_Set(PHASE_PIN, 0);
		GPIO_Set(ENABLE_PIN, 0);
		printf("off: %d\n", direction);
	}
	printf("%d\n", data);
	//printData(&jp1Controller);
	
}

void printData(JP1_Struct *jp1){
		printf("%d\n", jp1->data);
}
...
int main(void) {		
	...
	GPIO_Init();
	InitTimer(LOAD/2); 		// start at the off state 
